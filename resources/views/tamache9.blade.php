...
@section('content')
	...
	<div class="panel-body text-center">
		<form action="/tamache9" method="post" >
			{{ csrf_field() }}
			<table class="table table-striped table-bordered table-list">
				...
				@foreach($tamache9 as $mytamache9)

					<tr>

						<td class="hidden-xs">{{$var}}</td>
						<td>{{$mytamache9->mot}}</td>
						<td >
							{{$mytamache9->tamache9}}

						</td>
						<td style="width:150px;">{{(new DateTime($mytamache9->created_at))->format('Y-m-d')}}</td>
						<th style="width:100px;"><a href="" class="btn btn-success btn-lg">
    ...
    text-align: center;
    margin-bottom: -8px;" class="col-sm-12 form-group">
							<input class="form-control styleinput" id="name" name="tamache9" value="{{ old('tamache9') }}" placeholder="أكتب هنا الترجمة " type="text" >
							@if ($errors->has('tamache9'))
								<span class="help-block">
                                        <strong class="text-danger" style="font-size:20px;">{{ $errors->first('tamache9') }}</strong>
                                    </span>
							@endif
						</div>
					</td>
